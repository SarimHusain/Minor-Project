...
show_random_elements(squad_dataset["train"])
...
trainer = Trainer(
    model,
    args,
    train_dataset=datasets.Dataset.from_dict(tokenized_datasets["train"][:5]),
    eval_dataset=datasets.Dataset.from_dict(tokenized_datasets["validation"][:5]),
...
final_predictions = postprocess_qa_predictions(squad_dataset["validation"], validation_features, raw_predictions.predictions)
